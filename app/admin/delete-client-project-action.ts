...
export async function deleteClientProjectAction(projectId: string, clientId: string) {
  if (!z.string().uuid().safeParse(projectId).success || !z.string().uuid().safeParse(clientId).success)
    return { success: false, message: "Projeto ou cliente inválido." };
  const db = await createClient();
  const { data: { user }, error: authError } = await db.auth.getUser();
  if (authError || !user) return { success: false, message: "Entre novamente para continuar." };
  ...
  return { success: true, message: "Projeto excluído. Documentos e orçamentos continuam na área do cliente." };
}
