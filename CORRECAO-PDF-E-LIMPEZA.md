...
## Atualização: 1031a9fb — falha Vulkan persiste
A mera localização conjunta dos arquivos não resolveu o erro de produção. A causa completa ainda não foi demonstrada. O processo Chromium agora recebe LD_LIBRARY_PATH incluindo a pasta do runtime e VK_ICD_FILENAMES/VK_DRIVER_FILES apontando explicitamente ao manifesto empacotado. As variáveis são internas ao subprocesso, sem configuração manual na Vercel e sem alterar secrets.
Adicionado scripts/check-pdf-linux.mjs ao npm run build, logo após preparar o runtime. No Linux ele executa o renderer real para os três modelos vazios, valida os PDFs e interrompe o build se falhar. Não acessa banco, clientes, notificações ou e-mail. Windows registra explicitamente que esse teste Linux não foi executado. O ambiente de build e a Function ainda podem diferir; passar no build não comprova sozinho o funcionamento da Function.
Os nove testes de runtime/empacotamento passaram; lint dos arquivos alterados passou. A alteração continua pendente de verificação Linux. Se o build falhar, preservar o erro HAS_PDF_LAUNCH_FAILED e não remover a verificação para forçar publicação.
